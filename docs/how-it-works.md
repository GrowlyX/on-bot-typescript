...
The use of on-bot-kotlin eliminates all the delay layers listed above and is fully production-ready (meaning you don't need a full app reinstall once you are done with development). Additionally, on-bot-kotlin provides features that are not provided out-of-the-box by OnBot Java or Android Studio:
...
Since on-bot-kotlin uses Jsr223 (javax.script), Java's scripting engine, we are limited to only a few languages when writing OpMode scripts. We chose to keep native compatibility with Kotlin for on-bot-kotlin. If your team is not comfortable writing Kotlin code for OpModes, don't use this.
